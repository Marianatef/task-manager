...
export default function TaskForm({ onCreate, categories = [] }) {
  ...
  <input
    ...
    className="flex-1 px-2 py-1 border rounded w-full md:w-auto"
  />
  
  {categories.length > 0 && (
    <select value={cat} onChange={(e) => setCat(e.target.value)} className="px-2 py-1 border rounded w-full md:w-auto">
